...
def train_classifier(args_dict, train_subjects, val_subjects, test_subjects):
    ...
    for flooding_level in flooding_levels:
        ...
        if restore_file != 'None':
            summary_csv_path = os.path.join(restore_file, test_subjects[0], 'hypersearch_summary/hypersearch_summary.csv')  # find the best validation accuracy recordings
            subject_summary_df = pd.read_csv(summary_csv_path)
            subject_selected_setting = subject_summary_df.sort_values(by=['validation_accuracy'], ascending=False).iloc[0]
            restore_path = os.path.join(subject_selected_setting.experiment_folder, 'checkpoint', 'best_model.statedict')  # locate the best validation performance model .statedict file

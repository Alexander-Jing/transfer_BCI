...
def extract_experiment_setting(experiment_name):
    ...
    flooding_level[0] = experiment_name.split('_')[-3]
    flooding_level[1] = experiment_name.split('_')[-2]
    flooding_level[2] = experiment_name.split('_')[-1]
    
    #record to dict
    hyper_parameter_dict['flooding_level0'] = flooding_level[0]
    hyper_parameter_dict['flooding_level1'] = flooding_level[1]
    hyper_parameter_dict['flooding_level2'] = flooding_level[2] 
    
    #print values
    header = ' checking experiment '.center(100, '-')
    print(header)
    print('flooding_level0: {}; flooding_level1: {}; flooding_level2: {}'.format(flooding_level[0], flooding_level[1], flooding_level[2]))
...
def main(experiment_dir, summary_save_dir):
    ...
    with open(summary_filename, mode='w') as csv_file:
        ...
        fieldnames = ['validation_accuracy', 'test_accuracy', 'flooding_level0', 'flooding_level1', 'flooding_level2', 'performance_string', 'experiment_folder', 'status']
